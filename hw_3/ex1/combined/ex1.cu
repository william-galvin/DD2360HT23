/*
  Combined kernel implementation
...
__global__ void combined_kernel(unsigned int *input, unsigned int *bins,
  ...
  // Convert and clamp values to 127 in the same kernel
...
int main(int argc, char **argv) {
  ...
  start = clock();
  combined_kernel<<<blocksPerGrid, threadsPerBlock>>>(deviceInput, deviceBins, inputLength, NUM_BINS);
  printf("Combined kernel: %ld\n", clock() - start);
