...
int main(int argc, char **argv) {
  ...
  cusparseSpMatDescr_t A_desc;      // Mat descriptor needed by cuSPARSE
  cusparseDnVecDescr_t temp_desc;
  cusparseDnVecDescr_t tmp_desc;


  // Read the arguments from the command line
  if (argc != 3) {
    printf("Expected 3 arguments but found %d\n", argc);
    exit(EXIT_FAILURE);
  }
  dimX = atoi(argv[1]);
  nsteps = atoi(argv[2]);
  ...
  if (concurrentAccessQ) {
    ...
    gpuCheck(cudaMemPrefetchAsync(A, dimX * dimX * sizeof(double), device));
  ...
  cusparseDestroyDnVec(temp_desc);
  cusparseDestroyDnVec(tmp_desc);
